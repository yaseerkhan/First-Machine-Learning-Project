...
Now working with actual project, Library used for current project - SciKit-Learn, Algorithm used - DecisionTree
...
## Persisting Models : 
    ...
    # This stores our trained model into a file. Make sure there is nothing after this line of code
    # as we are storing our trained model in a file.
...
## Visualizing a Decision Tree : Decision Trees are the easiest to understand, we are gonna export our 
 # model in a visual format we'll see how this model makes prediction.

 import pandas as pd
 from sklearn.tree import DecisionTreeClassifier
 # tree object has a method for exporting our DecisionTree in a graphical format
...
# Explanation of Graph  
    ...
    It is a binary tree which means everynode can have a maximum of 2 childs, on top of each no we have a condition if that condition is true we go to the child node on the left side otherwise we go to the child node on the right side. 
